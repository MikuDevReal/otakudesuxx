// lib/detail.js
const { jikanGet, mapAnime, getOfficialStreaming } = require('./jikanClient');
...
async function getAnimeDetail(id) {
  ...
  // Link platform streaming LEGAL resmi (kalau MAL punya datanya) --
  // dibungkus try/catch sendiri, sama seperti episodeList/recommendedAnimeList.
  let officialStreaming = [];
  try {
    officialStreaming = await getOfficialStreaming(id);
  } catch (e) {
    console.warn(`Gagal ambil data streaming resmi utk anime ${id}:`, e.message);
  }
  ...
  return {
    ...
    // Daftar platform legal (Crunchyroll, Netflix, dll) tempat anime ini
    // bisa ditonton resmi. Bisa array kosong kalau MAL belum punya datanya
    // untuk judul ini -- UI harus tangani kondisi kosong dengan baik.
    officialStreaming,
    ...
    // link streaming apa pun -- tombol "Tonton" arahkan ke officialStreaming
    // di atas (platform legal), bukan ke sumber streaming tidak resmi.
    streamingNote: 'Endpoint ini hanya menyediakan info anime & judul episode, bukan video. Tombol "Tonton" diarahkan ke officialStreaming (platform legal).',
  };
}
