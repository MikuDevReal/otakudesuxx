...
// Daftar platform streaming LEGAL resmi untuk judul ini (kalau ada),
// sumber: MyAnimeList "Watch" section lewat Jikan. Bisa kosong kalau
// MAL belum punya data ini untuk judul tsb.
async function getOfficialStreaming(id) {
  const data = await jikanGet(`/anime/${id}/streaming`);
  return (data.data ?? []).map((s) => ({ name: s.name ?? '', url: s.url ?? '' }));
}

module.exports = { jikanGet, mapAnime, getOfficialStreaming };
